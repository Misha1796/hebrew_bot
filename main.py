...
# 1. Сначала получаем ключи из переменных окружения (Railway)
TOKEN = os.getenv("TOKEN")
GEMINI_KEY = os.getenv("GEMINI_KEY")

# 2. Только ПОТОМ настраиваем Gemini (теперь NameError исчезнет)
if GEMINI_KEY:
    genai.configure(api_key=GEMINI_KEY, transport="rest")
    model = genai.GenerativeModel("gemini-1.5-flash")
else:
    print("🚨 ОШИБКА: GEMINI_KEY не найден в переменных окружения!")
...
# Красивые названия для всех категорий из твоего words.json
MENU_TITLES = {
    "verbs": "🎬 Глаголы",
    "phrases": "💬 Фразы",
...
def get_trainer_categories():
    buttons = []
    # Теперь он видит все ключи: nouns, verbs, adjectives, connectors, past, future
...
async def start_translator(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("✍️ **Режим переводчика**\nНапиши фразу на русском.", reply_markup=get_back_kb(), parse_mode="Markdown")

@dp.message(BotStates.translator)
async def handle_translation(message: types.Message):
    if message.text and message.text.startswith('/'): return
    try:
        response = model.generate_content(f"Переведи на иврит: '{message.text}'. Напиши перевод и транскрипцию.")
...
async def show_theory_content(call: types.CallbackQuery):
    ...
    await call.message.edit_text(f"📘 {theory_data.get(topic_key, '...')}", reply_markup=get_back_kb(), parse_mode="Markdown")
