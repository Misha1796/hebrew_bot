...
user_data = {}    # {"user_id": {"correct":..., "mode":...}}
user_stats = {}   # {"user_id": {"correct":0,"wrong":0}}
error_queue = {}  # {"user_id": [item,...]}

# --- КЛАВИАТУРА ---
def build_keyboard(choices, include_reset=False):
    buttons = []
    for c in choices:
        display = f"{c}⠀"  # добавляем невидимый символ для растяжки
        buttons.append(InlineKeyboardButton(display, callback_data=c))
    rows = [buttons[i:i+2] for i in range(0, len(buttons), 2)]
    if include_reset:
        rows.append([InlineKeyboardButton("🔄 Сбросить статистику", callback_data="reset")])
...
def generate_question(user_id, mode):
    # сначала берем из очереди ошибок
...
async def handle_buttons(call: types.CallbackQuery):
    ...
    if call.data == correct:
        ...
        # добавляем слово в очередь ошибок 5 раз
        error_queue.setdefault(user_id, []).extend([item]*5)
