...
async def handle_translation(message: types.Message):
    ...
    try:
        prompt = f"Переведи на иврит: '{message.text}'. Дай: Перевод, Транскрипцию и род."
...
async def start_ai_chat(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("🎙 **Голосовой AI**\nПришли голосовое, и я отвечу голосом!", reply_markup=get_back_kb(), parse_mode="Markdown")
...
async def send_question(message, user_id, mode, feedback=""):
    ...
    if not available_words:
        text = f"{feedback}\n\n🎉 **Раздел пройден!**"
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🏠 В меню", callback_data="to_main")]])
